...
import Ingredients from '~/components/recipesComponents/Ingredients';
import ImageWithLike from '~/components/ui/ImageWithLike';
import { fetchRecipes, makeSlug } from '~/utils/helpers';
...
export default async function RecipePage({ params: { recipeId } }: Props) {
  ...
  //  };
  const recipeInfo: Recipe = await fetchRecipes({
    id: recipeId[0],
    isDynamic: true,
  });
